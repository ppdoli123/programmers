...
#              where c.car_id not in ( select c.car_id from car_rental_company_rental_history where datediff(h.end_date,'2022-11-01') >= 0 )) 
...
select distinct(c.car_id), h.history_id , h.start_date,h.end_date from car_rental_company_car c join car_rental_company_rental_history h on c.car_id = h.car_id
order by 4 desc
# SELECT 
#     C.CAR_ID, 
#     C.CAR_TYPE, 
